...
interface UseResourcesReturn {
  ...
  updateArticle: (id: string, updatedFields: Partial<ArticleType>) => Promise<ArticleType>;
...
const useArticles = (): UseResourcesReturn => {
  ...
  const createArticle = async (newResource: Omit<ArticleType, 'id'>): Promise<ArticleType> => {
    setError(null);
    try {
  ...
  const updateArticle = async (id: string, updatedFields: Partial<ArticleType>): Promise<ArticleType> => {
    setError(null);
    try {
      const res = await fetch(`${baseUrl}/article/${id}`, {
        method: 'PATCH',
