...
const Index = () => {
  ...
  const { fetchEmotionCategories, emotionCategories,createEmotionCategory,updateEmotionCategory, deleteEmotionCategory, loading, error } = useEmotionCategory();
  const [search, setSearch] = useState<string>("");
  const [emotionCategoriesFiltered, setEmotionCategoriesFiltered] = useState<EmotionCategory[]>([]);
 ...
 const createEmotionCategoryFormConfig: FieldConfig[] = [
  ...
  useEffect(() => {
    const filtered = emotionCategories.data.filter((c) => `${c.name}`.toLowerCase().includes(debouncedSearch.trim().toLowerCase()));
    ...
    <Box sx={{ width: "100%", display: "flex", flexDirection: "column", height: "100%" }}>
      ...
      {!loading && !error && (
        <Box sx={{ display: "flex", flexDirection: "column", height: "100%" }}>
          <HeaderGrid title="Liste des types de ressources" searchValue={search} onSearchChange={setSearch} onAddClick={() => setOpen(true)}/>
          <GridComponent rows={emotionCategoriesFiltered} columns={columns} loading={loading} hideFooter={true}  onRowDoubleClick={(params) => handleRowDoubleClick(params)} />
        <ModalEdition
            open={open}
